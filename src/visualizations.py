...
def scatterplots(finalData):
    ...
    ax2.set_title(f"Age and mean MGN vol for PD patients\ncorrelation = {round(corPD, 2)}", size=10)
    ax2.set_xlabel("Age of PD patients")
    ax2.set_ylim(60, 190)

    # set common y axis
    fig.text(0.06, 0.5, "Mean MGN volume", ha="center", va="center", rotation="vertical")

    fig.suptitle("Relationship between age and mean MGN volume per group\n", size=13)
    fig.subplots_adjust(top=0.80, left=0.15, wspace=0.5)
